...
from backend.app.core.models import Category, Customer, Product, SaleItem, StockLocation, Unit
...
def clean_database():
    ...
    with engine.begin() as connection:
        ...
        cleanup_order = ["sale_return_items", "purchase_return_items", "sale_returns", "purchase_returns", "cashbox_movements", "customer_payments", "customer_account_movements", "sale_payments", "sale_items", "sales", "stock_movements", "product_barcodes", "expense_payments", "expenses", "purchase_payments", "purchase_items", "purchases", "supplier_payments", "supplier_account_movements", "payment_methods", "customers", "suppliers", "products", "stock_locations", "units", "categories"]
        for table in cleanup_order:
            if table in tables:
                connection.exec_driver_sql(f'DELETE FROM "{table}"')
    seed_session = SessionLocal()
    try:
        for code, name, account_type, method_type, method_name in [("CASH", "النقد", "CASH", "CASH", "نقد"), ("WALLET", "المحفظة الإلكترونية", "WALLET", "WALLET", "محفظة إلكترونية")]:
            cash_box = seed_session.query(Cashbox).filter_by(code=code).first()
            if cash_box is None:
                cash_box = Cashbox(code=code, name=name, account_type=account_type, currency="BASE"); seed_session.add(cash_box); seed_session.flush()
...
def test_create_cash_sale_reduces_stock():
    ...
    try:
        ...
        assert InventoryService(session).get_balance(product.id, location.id) == Decimal("8.000"); session.commit()
...
def test_multi_line_sale_creates_all_items_and_reduces_each_stock():
    ...
    try:
        ...
        sale = SaleService(session).create_sale(document_no="S-MULTI-0001", business_date="2026-09-14", items=[{"product_id": first.id, "stock_location_id": location.id, "quantity": "2", "unit_price": "120", "discount": "0"}, {"product_id": second.id, "stock_location_id": location.id, "quantity": "3", "unit_price": "70", "discount": "0"}], payments=[{"payment_method": "CASH", "amount": "450"}], idempotency_key=str(uuid.uuid4()))
        assert sale.total == Decimal("450.00")
        assert session.scalar(select(SaleItem.id).where(SaleItem.sale_id == sale.id)) is not None
        assert session.query(SaleItem).filter_by(sale_id=sale.id).count() == 2
...
def test_mixed_payment_is_supported():
    ...
    try:
        ...
        customer = Customer(code="CUST-TEST-MIXED-001", name="test-customer-mixed-payment", phone="0000000000"); session.add(customer); session.flush()
        ...
        assert sale.total == Decimal("200.00"); assert sale.paid_amount == Decimal("150.00"); assert sale.credit_amount == Decimal("50.00"); assert sale.payment_status == "PARTIAL"; session.commit()
    finally: session.close()
...
def test_real_sale_creates_audit_log_automatically():
    ...
    try:
        ...
        assert sale.status == "CONFIRMED"; assert audit is not None; assert audit.entity_type == "SALE"; assert audit.entity_id == str(sale.id); session.rollback()
    finally: session.close()
