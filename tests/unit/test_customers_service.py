...
from backend.app.modules.customers.service import (
    ...
    CustomerError,
    CustomerService,
    DuplicateCustomerOperationError,
)
...
def test_customer_credit_increases_balance():
    session = get_session()
    try:
        customer, _, _ = setup_customer_product_stock(
            session,
...
def test_customer_payment_reduces_balance():
    session = get_session()
    try:
        customer, _, _ = setup_customer_product_stock(
            session,
