...
class InventoryPage(QWidget):
    ...
    def _build(self):
        ...
        transfer_form.addRow("الصنف", self._clone_product_selector())
        self.transfer_product = transfer_form.itemAt(0).widget()
        ...
        adjustment_form.addRow("الصنف", self._clone_adjustment_product_selector())
        self.adjustment_product = adjustment_form.itemAt(0).widget()
    ...
    def _new_product_selector(self):
        combo = QComboBox()
        combo.setObjectName("productSelector")
        return combo

    def _clone_product_selector(self):
        return self._new_product_selector()

    def _clone_adjustment_product_selector(self):
        return self._new_product_selector()
    ...
    def _load_selectors(self, session):
        ...
        selectors = [self.product, self.transfer_product, self.adjustment_product]
        ...
        self.location.clear()
        self.transfer_source.clear()
        self.transfer_target.clear()
        self.adjustment_location.clear()
        for item in locations:
            label = f"{item.code} — {item.name}"
            self.location.addItem(label, item.id)
            self.transfer_source.addItem(label, item.id)
            self.transfer_target.addItem(label, item.id)
            self.adjustment_location.addItem(label, item.id)
        for selector in selectors:
    ...
    def _sync_transfer_product(self):
        if hasattr(self, "transfer_product") and self.transfer_product.currentData() is not None:
    ...
    def _sync_adjustment_product(self):
        if hasattr(self, "adjustment_product") and self.adjustment_product.currentData() is not None:
    ...
    def add_opening_stock(self):
        ...
        try:
            InventoryService(session).add_stock(
                product_id=product_id, stock_location_id=location_id, quantity=quantity,
                unit_cost=unit_cost, business_date=date.today().isoformat(),
                idempotency_key=str(uuid4()), reference_type="OPENING_STOCK",
            )
            session.commit()
            self.quantity.setValue(0)
            QMessageBox.information(self, "تم الحفظ", "تمت إضافة الرصيد الافتتاحي وتسجيل الحركة.")
            self.refresh()
    ...
    def transfer_stock(self):
        ...
        source_id = self.transfer_source.currentData()
        target_id = self.transfer_target.currentData()
        quantity = Decimal(str(self.transfer_quantity.value()))
        if product_id is None or source_id is None or target_id is None:
            QMessageBox.warning(self, "بيانات ناقصة", "اختر الصنف ومخزني المصدر والاستلام.")
            return
        if source_id == target_id:
            QMessageBox.warning(self, "مخزن غير صحيح", "يجب أن يكون مخزن الاستلام مختلفًا عن مخزن المصدر.")
            return
        session = get_session()
        try:
            InventoryService(session).transfer_stock(
                product_id=product_id, source_location_id=source_id, target_location_id=target_id,
                quantity=quantity, unit_cost=Decimal(str(self.unit_cost.value())),
                business_date=date.today().isoformat(), idempotency_key=str(uuid4()),
            )
            session.commit()
            self.transfer_quantity.setValue(0)
            QMessageBox.information(self, "تم التحويل", "تم تحويل الكمية وتسجيل حركتي الخروج والدخول.")
            self.refresh()
        except Exception as exc:
            session.rollback()
    ...
    def adjust_stock(self):
        product_id = self.adjustment_product.currentData()
        location_id = self.adjustment_location.currentData()
        actual = Decimal(str(self.actual_quantity.value()))
        if product_id is None or location_id is None:
            QMessageBox.warning(self, "بيانات ناقصة", "اختر الصنف والمخزن أولاً.")
            return
        session = get_session()
        try:
            InventoryService(session).adjust_stock(
                product_id=product_id, stock_location_id=location_id, actual_quantity=actual,
                unit_cost=Decimal(str(self.unit_cost.value())), business_date=date.today().isoformat(),
                idempotency_key=str(uuid4()), reason=self.adjustment_reason.currentText().strip() or None,
            )
            session.commit()
            QMessageBox.information(self, "تمت التسوية", "تم تسجيل فرق الجرد كتسوية مخزنية دون حذف الحركات السابقة.")
            self.refresh()
        except Exception as exc:
            session.rollback()
            QMessageBox.critical(self, "تعذر الحفظ", str(exc))
    ...
    def refresh_balances(self, session=None):
        ...
        try:
            ...
            ).group_by(StockMovement.product_id, StockMovement.stock_location_id).subquery()
            ...
            for r, (product, location, quantity, value) in enumerate(rows):
                ...
                for c, item in enumerate([product.sku, product.name, location.name, f"{qty:,.3f}", f"{avg_cost:,.2f}", f"{total_value:,.2f}"]):
    ...
    def refresh_movements(self, session=None):
        ...
        try:
            ...
            for r, (movement, product, location) in enumerate(rows):
                ...
                action = movement_names.get(movement.movement_type, direction)
