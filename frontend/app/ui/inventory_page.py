...
from backend.app.modules.inventory.service import InventoryService, InsufficientStockError


class InventoryPage(QWidget):
    """Operational inventory dashboard, stock balances, opening stock and movement history."""
    ...
    def _build(self):
        ...
        tabs = QTabWidget()
        balances = QWidget()
        balance_layout = QVBoxLayout(balances)
    ...
    def refresh_movements(self, session=None):
        ...
        try:
            ...
            for r, (movement, product, location) in enumerate(rows):
                ...
                values = [movement.business_date, product.sku, product.name, location.name, direction, f"{movement.quantity:,.3f}", f"{movement.unit_cost:,.2f}"]
                for c, item in enumerate(values):
