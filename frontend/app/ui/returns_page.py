...
from PySide6.QtWidgets import QComboBox, QDoubleSpinBox, QFormLayout, QHBoxLayout, QLabel, QLineEdit, QMessageBox, QPushButton, QTabWidget, QVBoxLayout, QWidget
from sqlalchemy import select

from backend.app.core.database import get_session
from backend.app.core.models import Product, Purchase, PurchaseItem, Sale, SaleItem, Supplier, Customer
...
class ReturnsPage(QWidget):
    ...
    def _payment_methods(self, combo):
        combo.clear(); combo.addItem("بدون رد/استلام نقدي", None)
        for m in self._session.scalars(select(PaymentMethod).where(PaymentMethod.is_active.is_(True)).order_by(PaymentMethod.code)).all(): combo.addItem(f"{m.code} — {m.name}", m.code)
    ...
    def refresh(self):
        ...
        try:
            ...
            self._session=session; self._payment_methods(self.sale_refund); self._payment_methods(self.purchase_refund)
            self._load_sale_items(); self._load_purchase_items()
        finally:
            session.close(); self._session=None
        ...
        try:
            ...
            if sale_id:
                for item in session.scalars(select(SaleItem).where(SaleItem.sale_id==sale_id).order_by(SaleItem.id)).all():
                    product=session.get(Product,item.product_id); self.sale_item.addItem(f"{product.name} — المتاح {item.quantity}",item.id)
        ...
        try:
            ...
            if purchase_id:
                for item in session.scalars(select(PurchaseItem).where(PurchaseItem.purchase_id==purchase_id).order_by(PurchaseItem.id)).all():
                    product=session.get(Product,item.product_id); self.purchase_item.addItem(f"{product.name} — الكمية {item.quantity}",item.id)
        finally: session.close()
    ...
    def save_sale_return(self):
        ...
        try:
            ...
            session.commit(); self.sale_qty.setValue(0); self.sale_reason.clear(); self.refresh(); QMessageBox.information(self,"تم الحفظ",f"تم تأكيد مرتجع المبيعات {doc}.")
    ...
    def save_purchase_return(self):
        ...
        try:
            ...
            session.commit(); self.purchase_qty.setValue(0); self.purchase_reason.clear(); self.refresh(); QMessageBox.information(self,"تم الحفظ",f"تم تأكيد مرتجع المشتريات {doc}.")
        except Exception as exc: session.rollback(); QMessageBox.critical(self,"تعذر الحفظ",str(exc))
        finally: session.close()
