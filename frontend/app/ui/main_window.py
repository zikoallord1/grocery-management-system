...
from PySide6.QtWidgets import QFrame, QGridLayout, QHBoxLayout, QLabel, QMainWindow, QMessageBox, QPushButton, QSizePolicy, QStackedWidget, QVBoxLayout, QWidget
...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        header = QFrame(); header.setObjectName("header"); hl = QHBoxLayout(header); hl.setContentsMargins(22, 16, 22, 16); identity = QVBoxLayout(); title = QLabel(BRANDING.program_name); title.setObjectName("appTitle"); identity.addWidget(title); subtitle = QLabel("إدارة المبيعات والمشتريات والمخزون والحسابات من مكان واحد"); subtitle.setObjectName("appSubtitle"); identity.addWidget(subtitle); hl.addLayout(identity); hl.addStretch(); self.scanner_toggle = QPushButton("📷 قارئ الباركود"); self.scanner_toggle.setObjectName("scannerToggle"); self.scanner_toggle.clicked.connect(self._toggle_scanner); hl.addWidget(self.scanner_toggle); refresh = QPushButton("تحديث البيانات"); refresh.setObjectName("primaryButton"); refresh.clicked.connect(self.refresh_dashboard); hl.addWidget(refresh); root_layout.addWidget(header)
        ...
        self.barcode_scanner = BarcodeScannerWidget(root); self.barcode_scanner.barcode_detected.connect(self._handle_barcode); self.barcode_scanner.hide()

    def resizeEvent(self, event):
        super().resizeEvent(event)
        if hasattr(self, "barcode_scanner"): self._position_scanner()

    def _position_scanner(self):
        margin = 18
        x = self.centralWidget().width() - self.barcode_scanner.width() - margin
        self.barcode_scanner.move(max(margin, x), margin)
        self.barcode_scanner.raise_()

    def _toggle_scanner(self):
        if self.stack.currentWidget() is not self._pages.get("المبيعات"): self._show_page("المبيعات")
        self.barcode_scanner.toggle()
        if self.barcode_scanner.isVisible(): self._position_scanner()

    def _handle_barcode(self, code):
        sales_page = self._pages.get("المبيعات")
        if sales_page is None or self.stack.currentWidget() is not sales_page: return
        session = get_session()
        try:
            product = session.scalar(select(Product).join(ProductBarcode, ProductBarcode.product_id == Product.id).where(Product.is_active.is_(True), ProductBarcode.is_active.is_(True), ProductBarcode.barcode == code))
            if product is None:
            ...
            if index < 0:
                ...
                return
            sales_page.product.setCurrentIndex(index); sales_page.quantity.setValue(1); sales_page.add_line(); self.barcode_scanner.status.setText(f"تمت إضافة: {product.name}"); sales_page.product.setFocus()
    ...
    def _show_dashboard(self): self.stack.setCurrentWidget(self.dashboard); self.barcode_scanner.hide(); self.refresh_dashboard()

    def _show_page(self, name):
        page = self._pages[name]; page.refresh() if hasattr(page, "refresh") else None; self.stack.setCurrentWidget(page)
        if name == "المبيعات" and self.barcode_scanner.isVisible(): self._position_scanner()
        elif name != "المبيعات": self.barcode_scanner.hide()
    ...
    def closeEvent(self, event):
        if hasattr(self, "barcode_scanner"): self.barcode_scanner.close_camera()
    ...
    def _stylesheet():
        ...
        #scannerStatus { color: #61758a; font-size: 11px; }
        #scannerPreview { background: #111827; color: white; border-radius: 6px; }
        #scannerCode { background: #eef2f6; color: #17324d; border-radius: 5px; padding: 3px 6px; font-weight: 700; }
