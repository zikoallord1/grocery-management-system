...
class SummaryCard(QFrame):
    def __init__(self, title, value, parent=None):
        ...
        layout = QVBoxLayout(self); layout.setContentsMargins(18, 14, 18, 14); layout.setSpacing(6)
        label = QLabel(title); label.setObjectName("summaryLabel"); label.setWordWrap(True); layout.addWidget(label)
        self.value_label = QLabel(value); self.value_label.setObjectName("cardValue"); layout.addWidget(self.value_label)
...
class SettingsPage(QFrame):
    ...
    def __init__(self, parent=None):
        super().__init__(parent); self.setObjectName("panel"); self.setLayoutDirection(Qt.RightToLeft)
        layout = QVBoxLayout(self); layout.setContentsMargins(26, 24, 26, 24); layout.setSpacing(14)
        title = QLabel("الإعدادات"); title.setObjectName("pageTitle"); layout.addWidget(title)
        desc = QLabel("مركز إعداد النظام: بيانات البقالة، المستخدمون والصلاحيات، تسجيل النسخة، الطابعة والفواتير، النسخ الاحتياطي والمزامنة."); desc.setWordWrap(True); desc.setObjectName("pageDescription"); layout.addWidget(desc)
        for text in ["بيانات البقالة والهوية", "المستخدمون والصلاحيات", "تسجيل النسخة والترخيص", "الطابعة والفواتير", "النسخ الاحتياطي والاستعادة", "المزامنة والاتصال"]:
            b = QPushButton(text); b.setObjectName("actionButton"); layout.addWidget(b)
        layout.addStretch(); back = QPushButton("العودة إلى الرئيسية"); back.setObjectName("secondaryButton"); back.clicked.connect(self.back_requested.emit); layout.addWidget(back, alignment=Qt.AlignLeft)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self._cards = {}; self._pages = {}; self._nav_buttons = {}; self._notification_offset = 0; self._build_ui()
    ...
    def _build_ui(self):
        ...
        side_title = QLabel(BRANDING.program_name); side_title.setObjectName("sideTitle"); side_title.setWordWrap(True); side_layout.addWidget(side_title)
        side_subtitle = QLabel("القائمة الرئيسية"); side_subtitle.setObjectName("sideSubtitle"); side_layout.addWidget(side_subtitle); side_layout.addSpacing(8)
        ...
        self._specs = specs
        for label in specs:
            button = QPushButton(label); button.setObjectName("navButton"); button.setProperty("active", label == "الرئيسية"); button.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed); button.clicked.connect(lambda checked=False, name=label: self._show_page(name)); side_layout.addWidget(button); self._nav_buttons[label] = button
        side_layout.addStretch()
        refresh_side = QPushButton("تحديث البيانات"); refresh_side.setObjectName("primaryButton"); refresh_side.clicked.connect(self.refresh_dashboard); side_layout.addWidget(refresh_side)
        ...
        header = QFrame(); header.setObjectName("header"); hl = QHBoxLayout(header); hl.setContentsMargins(22, 16, 22, 16); hl.setSpacing(12)
        ...
        subtitle = QLabel("إدارة المبيعات والمشتريات والأصناف والمخزون والحسابات من مكان واحد"); subtitle.setObjectName("appSubtitle"); subtitle.setWordWrap(True); identity.addWidget(subtitle); hl.addLayout(identity, 1)
        content_layout.addWidget(header)

        ticker = QFrame(); ticker.setObjectName("notificationBar"); ticker_layout = QHBoxLayout(ticker); ticker_layout.setContentsMargins(12, 5, 12, 5); ticker_layout.setSpacing(8)
        notification_title = QLabel("التنبيهات"); notification_title.setObjectName("notificationTitle"); ticker_layout.addWidget(notification_title)
        self._notification_label = QLabel(); self._notification_label.setObjectName("notificationText"); self._notification_label.setAlignment(Qt.AlignCenter); ticker_layout.addWidget(self._notification_label, 1); content_layout.addWidget(ticker)
        self._notification_messages = ["مرحبًا بك في نظام إدارة البقالات — تابع المبيعات والمشتريات والمخزون والحسابات من مكان واحد", "تذكير: راجع الأصناف منخفضة المخزون قبل بدء يوم البيع", "تنبيه: احفظ الفواتير واعتمد العمليات المالية بعد مراجعتها", f"{BRANDING.program_name} — {BRANDING.designer_credit}", f"حقوق التصميم والتنفيذ محفوظة — {BRANDING.contact_text}", f"للتواصل وطلب البرنامج أو المساعدة: {BRANDING.phone_display} — واتساب متاح"]
        ...
        scroll = QScrollArea(); scroll.setObjectName("contentScroll"); scroll.setWidgetResizable(True); scroll.setFrameShape(QFrame.NoFrame); scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff); scroll.setWidget(self.stack); content_layout.addWidget(scroll, 1)
        footer = QFrame(); footer.setObjectName("footerPanel"); fl = QHBoxLayout(footer); fl.setContentsMargins(10, 5, 10, 5)
        credit = QLabel(f"{BRANDING.designer_credit} | {BRANDING.contact_text}"); credit.setWordWrap(True); fl.addWidget(credit, 1)
        ...
        wa = QPushButton("واتساب"); wa.setObjectName("linkButton"); wa.clicked.connect(lambda: open_url(BRANDING.whatsapp_uri)); fl.addWidget(wa); content_layout.addWidget(footer)
        root_layout.addWidget(content, 1); self.setCentralWidget(root); self.setStyleSheet(self._stylesheet())
    ...
    def _dashboard_page(self):
        page = QWidget(); layout = QVBoxLayout(page); layout.setContentsMargins(10, 10, 10, 18); layout.setSpacing(18)
        ...
        quick = QFrame(); quick.setObjectName("panel"); ql = QVBoxLayout(quick); ql.setContentsMargins(20, 18, 20, 18); ql.setSpacing(14); h = QLabel("العمليات الرئيسية"); h.setObjectName("pageTitle"); ql.addWidget(h)
        row = QGridLayout(); row.setHorizontalSpacing(12); row.setVerticalSpacing(12)
        for i, (label, target) in enumerate([("فاتورة بيع جديدة", "المبيعات"), ("فاتورة شراء جديدة", "المشتريات"), ("إضافة صنف", "الأصناف"), ("قبض من عميل", "العملاء"), ("سداد مورد", "الموردون"), ("تسجيل مصروف", "المصروفات"), ("الصناديق والحسابات", "الصناديق والحسابات"), ("المرتجعات", "المرتجعات")]):
            button = QPushButton(label); button.setObjectName("actionButton"); button.clicked.connect(lambda checked=False, name=target: self._show_page(name)); row.addWidget(button, i // 4, i % 4)
    ...
    def _move_notification(self):
        if not self._notification_messages: return
        self._notification_label.setText(self._notification_messages[self._notification_offset % len(self._notification_messages)]); self._notification_offset += 1
    ...
    def _stylesheet():
        ...
        #sideNavigation { background: #17324d; border-radius: 12px; }
        #sideTitle { color: white; font-size: 18px; font-weight: 700; padding: 8px; }
        #sideSubtitle { color: #dce8f2; padding: 4px 8px; }
        ...
        #navButton { min-height: 42px; padding: 0 12px; text-align: right; border-radius: 7px; background: transparent; color: white; border: 1px solid transparent; }
        #navButton:hover { background: #244b6b; }
